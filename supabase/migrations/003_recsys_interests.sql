-- MLE-friendly interest graph for recommendation pipelines
-- - interest_tags: stable taxonomy (content-side features / cold-start)
-- - user_interest_preferences: current explicit preference edges (serving / feature store)
-- - user_interest_events: append-only feedback log (training / offline eval)
...
create index if not exists user_interest_events_type_time_idx
  on public.user_interest_events (event_type, occurred_at desc);

-- Wide/flat export for training jobs (joins taxonomy + current prefs)
...
-- Event stream export for sequential / pairwise training
...
-- Seed canonical taxonomy (slug is the stable join key for embeddings / models)
...
create policy "Users read own interest preferences"
  on public.user_interest_preferences
  for select
...
create policy "Users read own interest events"
  on public.user_interest_events
  for select
  using (auth.uid() = user_id);
...
-- Atomic onboarding write: preferences snapshot + event log + profile denorm
...
begin
  ...
  if cardinality(selected_ids) <> cardinality(interest_slugs) then
    raise exception 'one or more interest slugs are invalid';
  end if;

  -- Log deselects for interests being removed (training signal)
  for removed in
